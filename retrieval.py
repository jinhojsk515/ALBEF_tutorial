...
def evaluate(model, data_loader, tokenizer, device, config):
    ...
    for i in range(0, num_text, text_bs):
        ...
        text_input = tokenizer(text, padding='max_length', truncation=True, max_length=150, return_tensors="pt").to(
            device)
    ...
    for image,_, label in data_loader:
        ...
        image_feat = model.visual_encoder(image,return_dict=True).last_hidden_state[:, 0, :]
    ...
    for i, sims in enumerate(sims_matrix[start:end]):
        ...
        print('kkk',text_feats[topk_idx].size())
        output_image = model.text_encoder(encoder_embeds = encoder_output,
        ...
        output_text = model.text_encoder(encoder_embeds=text_feats[topk_idx],
    ...
    for i, sims in enumerate(sims_matrix[start:end]):
        ...
        output_text = model.text_encoder(encoder_embeds=text_feats[start + i].repeat(config['k_test'], 1, 1),
...
def main(args, config):
    ...
    dataset_val = MIMIC_CXRDataset("./data/MIMIC_CXR/Train.jsonl", transform=test_transform,data_length=[0,100],shuffle=False)
    dataset_test = MIMIC_CXRDataset("./data/MIMIC_CXR/Test.jsonl", transform=test_transform,data_length=[0,100],shuffle=False)
    ...
    score_val_i2t, score_val_t2i,labels_val = evaluate(model_without_ddp, val_loader, tokenizer, device, config)
    score_test_i2t, score_test_t2i,labels_test = evaluate(model_without_ddp, test_loader, tokenizer, device, config)
    print('VALIDATION')
    val_stats = itm_eval(score_val_i2t,score_val_t2i,labels_val)
    print('TEST')
    test_stats = itm_eval(score_test_i2t, score_test_t2i,labels_test)
